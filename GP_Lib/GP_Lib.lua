--[[
GP_Lib - Shared UI utilities for GetParanoid addons.

Provides a draggable config window and widget helpers (checkbox, slider,
dropdown, edit box). No dependency on InterfaceOptionsFrame.

Usage:
    local win = GP_Lib:CreateWindow("My Settings", 500, 400)
    GP_Lib:AddCheckbox(win, "Enable thing", x, y, getter, setter)
    GP_Lib:AddSlider(win, "Font Size: %d", x, y, 24, 128, 2, getter, setter)
    GP_Lib:AddDropdown(win, "Channel", x, y, {"SAY","PARTY"}, getter, setter)
    GP_Lib:AddEditBox(win, "Message", x, y, 300, getter, setter)
    win:Show() / win:Hide()
--]]
...
-- Widgets placed inside it participate in the Save / refresh lifecycle.
function GP_Lib:CreateWindow(title, width, height)
    ...
    -- Drag handle (title bar area only)
    ...
    -- Content sub-frame (widget anchor)
    ...
    -- Load current values into widgets when shown
...
-- fmt    : format string for the title label, receives the current value e.g. "Font Size: %d"
...
-- For dropdowns, also attaches to the inner button so hover always fires.
function GP_Lib:SetTooltip(widget, text)
    ...
    -- UIDropDownMenuTemplate: the inner button intercepts mouse events,
    -- so we attach there too so hovering the button also shows the tip.

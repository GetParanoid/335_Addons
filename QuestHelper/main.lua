...
QH_Event("ADDON_LOADED", function (addonid)
  ...
  QH_Timeslice_Add(function ()
    ...
    -- Warperia Status Message
    local build_info = GetBuildInfo()
    local status_color = "|cff00ff00" -- Green for "Working"
    local status_text = "Working"
    
    -- Check if this is WOTLK 3.3.5
    if build_info and build_info:match("^3%.3%.5") then
      status_color = "|cff00ff00" -- Green
      status_text = "Optimized for WOTLK 3.3.5"
    else
      status_color = "|cffffff00" -- Yellow for other versions
      status_text = "Status: Active"
    end
    
    -- Display the Warperia message
    self:TextOut(string.format("%sQuestHelper %s|r - Updated by |cff00bfffWarperia|r", 
                              status_color, status_text))
    self:TextOut(string.format("Visit |cff00bfffwarperia.com|r for more addons!"))

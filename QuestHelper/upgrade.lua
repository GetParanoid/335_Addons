...
function QuestHelper_BuildZoneLookup()
  ...
  if GetMapContinents and GetMapZones then
    ...
    -- WOTLK 3.3.5 FIX: Skip zone validation for WOTLK to prevent errors
    local gameVersion = GetBuildInfo()
    if gameVersion and string.find(gameVersion, "3%.3%.5") then
      -- For WOTLK 3.3.5, use the static zone data without validation
      ...
      for base_name, i in pairs(original_lookup) do
        local index = i[1]
        local pair = {i[2], i[3]}
        local name = QuestHelper_Zones[pair[1]] and QuestHelper_Zones[pair[1]][pair[2]]
    ...
    -- Original validation code for other versions
